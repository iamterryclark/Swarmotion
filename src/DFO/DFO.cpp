...
DFO::DFO(int population){
    for (int i = 0 ; i < population; i++){
        swarm.push_back(Fly());
        swarm[i].setPos( { ofRandom(120),  ofRandom(90) } );
    }
...
void DFO::find(ofPixels &searchSpace){
        ...
        switch(topology){
            ...
            case 2: //Closest neighbour
                ...
                for (int i = 0; i < swarm.size(); i++) {
                    ...
                    double d = fly.getDistance(swarm[i]);
                    if (d < minDL) {
                        minDL = d;
                        leftN = index;
                    }
...
void DFO::render(ofPixels &searchSpace){
    ofPushStyle();
    for(int i = 0; i < swarm.size(); i++) {
...
float DFO::highestVel(ofVec2f _velocity){
...
double DFO::output(){
    ...
    for (int i = 0 ; i < swarm.size(); i++){
        ofVec2f dist = swarm[i].getDistance2vec(swarm[bestIndex]);
